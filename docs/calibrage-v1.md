...
```
Note Finale = (Note de Base × Curseur A) − (PER des absents × Curseur B) − Malus Calendrier + Bonus Draft + Bonus/Malus Transferts
```
...
- **`player_sample_size_threshold` : 5 (nouveau, Étape 6 tardive, 2026-08-28), non validé par simulation.** Garde-fou petit échantillon **individuel** (par joueur, sur son nombre de matchs joués cette saison) : sous ce seuil, le PER/MPG N-1 du joueur (`PreviousSeasonPlayerStat`) est utilisé à la place de sa valeur courante — sans quoi un titulaire établi hérite d'un PER basé sur 1 à 3 matchs, statistiquement peu fiable, dès l'import du roster de la saison courante. **Distinct de `EARLY_SEASON_GAME_THRESHOLD` (10, non configurable, niveau ÉQUIPE)** : une équipe peut avoir dépassé ses 10 premiers matchs pendant qu'un joueur précis (retour de blessure, appelé en cours de saison) reste sous ce seuil individuel — les deux mécanismes sont indépendants. Valeur par défaut choisie plus basse que le seuil équipe (5 vs 10) : les statistiques individuelles agrégées dans le PER (box-score par match) se stabilisent plus vite qu'un pourcentage de victoires d'équipe, un seuil plus court suffit donc à écarter les cas extrêmes (1 à 3 matchs) sans basculer inutilement en N-1 un joueur dont l'échantillon courant est déjà raisonnablement informatif. Comme `transfer_impact_multiplier`, à revalider dès que possible avec de vraies données de saison.
...
- **`transfer_impact_multiplier` : défaut 0.4 (Étape 6bis), non validé par simulation.** Aligné par cohérence sur `per_impact_multiplier` (même échelle, même nature d'ajustement — un joueur qui change d'équipe l'été), mais contrairement aux autres curseurs de cette page, sa valeur n'a **pas** été confrontée à une distribution de scénarios simulés (la simulation de l'Étape 6 ne modélisait pas de transferts). À valider au même titre que le reste dès que possible.
...
- Le terme "Bonus/Malus Transferts" (implémenté à l'Étape 6bis) utilise un curseur (`transfer_impact_multiplier`) non validé par simulation — voir ci-dessus.
- Le garde-fou petit échantillon individuel (`player_sample_size_threshold`) est également non validé par simulation — voir ci-dessus.
